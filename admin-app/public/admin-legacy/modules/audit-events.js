...
export const auditEventsModule = {
  ...
  async mount(ctx) {
    ...
    content.innerHTML = `
      ...
      <section class="panel-card sticky-filters">
        <div class="filters-grid">
          <label class="field"><span>Source</span><select id="audit-source"><option value="all">All</option><option value="admin_action_logs">Admin actions</option><option value="admin_auth_events">Admin auth events</option><option value="business_review_audit_log">Business review audit</option><option value="stripe_webhook_events">Stripe webhooks</option><option value="plaid_webhook_events">Plaid webhooks</option><option value="plaid_event_logs">Plaid event logs</option></select></label>
        </div>
      </section>
    ...
    const load = async () => {
      const [adminActions, adminAuthEvents, businessAudit, stripeHooks, plaidHooks, plaidEvents] = await Promise.all([
