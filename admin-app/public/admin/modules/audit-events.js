...
export const auditEventsModule = {
  ...
  async mount(ctx) {
    ...
    content.innerHTML = `
      ...
      <section class="panel-card sticky-filters">
        <div class="filters-grid">
          <label class="field"><span>Source</span><select id="audit-source"><option value="all">All</option><option value="admin_action_logs">Admin actions</option><option value="admin_auth_events">Admin auth events</option><option value="business_review_audit_log">Business review audit</option><option value="stripe_webhook_events">Stripe webhooks</option><option value="plaid_webhook_events">Plaid webhooks</option><option value="dots_webhook_events">Dots webhooks</option><option value="tremendous_webhook_events">Tremendous webhooks</option><option value="plaid_event_logs">Plaid event logs</option></select></label>
        </div>
      </section>
    ...
    const load = async () => {
      const [adminActions, adminAuthEvents, businessAudit, stripeHooks, plaidHooks, dotsHooks, tremendousHooks, plaidEvents] = await Promise.all([
        ...
        safeLoad(() => runtime.client.from("admin_auth_events").select("id,event_name,endpoint,actor_email,actor_role,outcome,reason,status_code,created_at,metadata").order("created_at", { ascending: false }).limit(100)),
      ...
      rows = [
        ...
        ...adminAuthEvents.map((row) => ({ id: `aa:${row.id}`, source: "admin_auth_events", created_at: row.created_at, type: row.event_name, detail: `${row.outcome} ${row.status_code || ""}`.trim(), raw: row })),
