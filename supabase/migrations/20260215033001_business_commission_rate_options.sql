-- Allow only admin-selected business commission rates (10% or 15%) and
-- apply that rate to receipt commission calculations.
...
where coalesce(commission_rate_cents, 0) not in (100, 150);
...
begin
  ...
  if total_cents is not null and total_cents > 0 then
    new.commission_due_cents := round((total_cents::numeric) * (commission_rate_bps::numeric) / 10000)::integer;
  end if;
...
before insert or update of receipt_total_cents, commission_due_cents, business_id on public.receipt_uploads
...
declare
  ...
  cashback_rate_bps integer := 5000;
