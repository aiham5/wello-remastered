...
declare
  ...
  v_raw_cashback_cents integer := 0;
...
begin
  ...
  v_raw_cashback_cents := floor(
    (v_eligible_total_cents::numeric) * (v_effective_cashback_rate_bps::numeric) / 10000
  )::integer;

  v_cashback_cents := case
    when v_is_trade_business then least(v_raw_cashback_cents, 100000)
    else v_raw_cashback_cents
  end;

  if v_is_trade_business and v_cashback_cents < v_raw_cashback_cents then
    v_cashback_basis := 'cashback_amount_capped';
  end if;
