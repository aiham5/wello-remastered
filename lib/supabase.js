...
const safeStorage = {
  getItem: async (key) => {
    try {
      return await withTimeout(AsyncStorage.getItem(key), 3000, "storage.get");
    } catch (error) {
      storageMode = "memory";
      storageFailureReason = error?.message || "storage.get failed";
  ...
  setItem: async (key, value) => {
    try {
      await withTimeout(AsyncStorage.setItem(key, value), 3000, "storage.set");
    } catch (error) {
      storageMode = "memory";
      storageFailureReason = error?.message || "storage.set failed";
      console.warn("AsyncStorage setItem timeout", key, error?.message);
    }
  },
  removeItem: async (key) => {
    try {
      await withTimeout(AsyncStorage.removeItem(key), 3000, "storage.remove");
    } catch (error) {
      storageMode = "memory";
      storageFailureReason = error?.message || "storage.remove failed";
      console.warn("AsyncStorage removeItem timeout", key, error?.message);
    }
  },
  getAllKeys: async () => {
    try {
      return await withTimeout(AsyncStorage.getAllKeys(), 3000, "storage.keys");
    } catch (error) {
      storageMode = "memory";
      storageFailureReason = error?.message || "storage.keys failed";
...
export const refreshAccessTokenWithRefreshToken = async (timeoutMs = 6000) => {
  const supabaseUrl = getEnv("EXPO_PUBLIC_SUPABASE_URL");
